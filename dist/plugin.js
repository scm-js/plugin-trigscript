...
function planOnce(ts, checker, arrow, options, forced) {
    ...
    if (target && root && ts.isIdentifier(root) && (ts.isElementAccessExpression(target) || ts.isPropertyAccessExpression(target))) {
      ...
      if (decl && ts.isVariableDeclaration(decl) && decl.initializer && inside(decl)) {
        ...
        if (checker.isArrayType(type) || checker.isTupleType(type)) {
          ...
          const member2 = ts.isPropertyAccessExpression(target) ? target.name.text : void 0;
          ...
          if (member2 === "push" || member2 === "pop" || member2 === "length" || atLength) plan.grows.add(decl);
        }
      }
    }
  };
  ...
  const callback = (fn, items) => {
    ...
    if (ts.isBlock(fn.body)) {
      items.push({ kind: "block", items: block2(fn.body.statements) });
      return;
    }
    const inner = [];
    value(fn.body, inner);
  ...
  const classItems = (c2) => {
    ...
    for (const m of c2.members) {
      ...
      else if ((ts.isConstructorDeclaration(m) || ts.isMethodDeclaration(m) || ts.isAccessor(m)) && m.body) items.push({ kind: "block", items: block2(m.body.statements) });
    }
    return items;
  };
...
function serializeIr(programs, strings, input = null, settings = {}) {
  ...
  const stmt = (s) => {
    switch (s.kind) {
      ...
      case "action":
        return { ...s, record: action2(s.record), ...s.variables ? { variables: s.variables.map((v) => ({ ...v, expr: expr(v.expr) })) } : {}, ...s.text ? { text: text(s.text) } : {} };
      case "centerLocation":
...
function typeNumbers(program) {
  ...
  const texts = { num: (e) => num3(e)[0], bool: (e) => bool(e), call: (c2) => {
    call(c2);
    return c2;
  }, text };
  const unit = (u) => {
...
var Structured = class _Structured {
  ...
  declare(list) {
    ...
    for (const d of list.declarations) {
      ...
      if (recordsOf) {
        const records2 = this.declareRecords(d.name.text, init, recordsOf, d, rowClass);
        if (records2) this.scope.bind(d, records2);
        continue;
      }
  ...
  textCells(of, key, index) {
    const [addr, block2, chars] = TEXT_PARTS.map((part) => of.fields.get(`${key} ${part}`));
    return { kind: "textAt", addr, block: block2, chars, index };
  }
  ...
  newRow(of, made, shape, held) {
    ...
    out.fill.push((records2, index) => {
      const self = this.rowOf(records2, index);
      this.onRow.set(self, { of: records2, index, shape });
  ...
  recordTables(e) {
    ...
    const records2 = { kind: "records", name, fields };
    this.recordLists.set(list, records2);
    return records2;
  }
  rowIndex(of, e) {
  ...
  recordsFrom(name, shape, rows, init, at, grows, cls) {
    ...
    const records2 = { kind: "records", name, fields, ...cls ? { cls } : {}, ...plain ? {} : { shape } };
    ...
    this.releaseFrom(records2, num(0), at);
    ...
    for (const [k, row] of rows.entries()) for (const fill of row.fill) if (!fill(records2, num(k))) return null;
    return records2;
  }
  ...
  declareTexts(name, initializer, at) {
    ...
    const records2 = this.recordsFrom(name, shape, rows, init, at, this.body.plan.grows.has(at), void 0);
    ...
    return { kind: "texts", name, rows: records2 };
  }
  ...
  memberAt(e, pick, quiet = false) {
    ...
    if (obj.kind === ts.SyntaxKind.SuperKeyword) {
      ...
      const member3 = this.memberOf(cls, (m) => !this.isStatic(m) && pick(m, name), 1);
      if (!member3) {
        if (quiet) return void 0;
        this.c.error(e, `What ${this.className(cls)} extends has no ${name}().`);
        return null;
      }
      return { member: member3, self: self2, name: `${this.className(member3.parent)}.${name}` };
    }
    const named = this.classOf(obj);
    if (named) {
      const member3 = this.memberOf(named, (m) => this.isStatic(m) && pick(m, name));
      if (!member3) {
        if (quiet) return void 0;
        this.c.error(e, `${this.className(named)} has no static ${name}().`);
        return null;
      }
      return { member: member3, name: `${this.className(named)}.${name}` };
    }
    const before = [];
    ...
    const member2 = this.memberOf(self.cls, (m) => !this.isStatic(m) && pick(m, name));
    if (!member2) {
      if (quiet || self.fields.has(name)) return void 0;
      this.c.error(e, `${this.className(self.cls)} has no method ${name}().`);
      return null;
    }
    return { member: member2, self, name: `${this.className(self.cls)}.${name}`, ...before.length ? { before } : {} };
  }
  /* ── Functions ── */
...
var ProgramRun = class {
  ...
  constructor(sim, index, program) {
    ...
    for (const decl of program.arrays ?? []) this.arrays.set(decl.id, { decl, room: 0, cells: decl.values ? [...decl.values] : decl.dynamic ? [] : new Array(decl.length).fill(decl.kind === "number" ? 0 : false) });
    this.body = this.run();
  }
  *run() {
  ...
  *unit(e) {
    switch (e.kind) {
      ...
      case "unitAt": {
        ...
        yield* this.num(e.uid);
        return ptr >= 1 ? this.sim.units[ptr - 1] ?? null : null;
      }
      case "call": {
  ...
  *num(e) {
    switch (e.kind) {
      ...
      case "unitPart": {
        ...
        return u ? this.sim.units.indexOf(u) + 1 : 0;
      }
      case "tableRead":
...
var ProgramSimulation = class {
  ...
  text(name, program = 0) {
    return this.runs[program]?.textValue(name);
  }
  /** An array's cells, by its name in the source. */
  list(name, program = 0) {
    return this.runs[program]?.list(name);
  }
};
