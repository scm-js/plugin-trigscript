...
import { declarations, type At, type BoolExpr, type Call, type NumExpr, type Program, type Stmt } from "./ir";
...
function expressions(body: Stmt[], visit: (e: NumExpr | BoolExpr) => void) {
  ...
  const call = (c: Call) => { for (const p of c.params) expr(p.init); c.body.forEach(stmt); };
  const stmt = (s: Stmt) => {
    switch (s.kind) {
      case "declare": if (!s.failed) expr(s.init); break;
...
function checkWidths(body: Stmt[], out: ProgramDiagnostic[]) {
  ...
  const fits = (id: string, value: NumExpr | BoolExpr, at: At) => {
  ...
  const stmt = (s: Stmt) => {
    switch (s.kind) {
      ...
      case "while": case "do": s.body.forEach(stmt); break;
